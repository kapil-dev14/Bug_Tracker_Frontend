...
import { BUG_STATUSES, BUG_PRIORITIES, STATUS_STYLES, PRIORITY_STYLES, normalize } from "../../utils/constants";
    ...
    <div className="fixed inset-0 z-40 flex justify-end bg-ink/40">
      <div className="flex h-full w-full max-w-lg flex-col bg-card shadow-panel">
        ...
        <div className="flex-1 overflow-y-auto p-5">
          <h2 className="mb-1 font-display text-lg font-semibold text-ink">{bug.title}</h2>
          <p className="mb-5 text-xs text-slate-muted">
            Reported {formatDateTime(bug.createdAt)} by {bug.createdBy?.username || "unknown"}
          </p>

          <p className="mb-6 whitespace-pre-wrap text-sm text-slate-text">{bug.description}</p>
          ...
          <div className="mb-6 grid grid-cols-2 gap-3">
            <Select
              ...
              options={BUG_STATUSES.map((s) => ({ value: s, label: s.replace("_", " ") }))}
            />
            <Select
              label="Priority"
              value={bug.priority}
